...
// functions for users login , logout, user - information and session - handling
	...
	function validate_login($dbMySQL) {
...
//print_r($SqlStr);
        ...
        if (mysqli_num_rows($res) > 0) {
			// login successfull
                $dbresult = mysqli_fetch_assoc ($res);
	...
	function check_session ($dbMySQL) {
		...
		*/
		...
		if (!isset($_SESSION['started'])) {
			...
			$_SESSION['password'] = "";
			$_SESSION['group'] = 0;
			$_SESSION['msg'] = "Session abgelaufen, bitte erneut anmelden.";
			$_SESSION['started'] = 1;
		...
		if ($_SESSION['logged_in'] == 0) {
			...
			if (validate_login($dbMySQL) == true){
				...
				$_SESSION['msg'] = "Anmeldung ok";
				
			}
			else {

				$_SESSION['msg'] = "Benutzename / Passwort nicht korrekt";
			}
		}
